/*
The basis for this file was a copy of fr.gnodet.githubfs.GitHubPath
...
public class LibFilePath implements Path {
    private final LibFileFileSystem fileSystem;
    private final byte[] path;
    private volatile int[] offsets;
    private volatile int hash = 0;
    private volatile byte[] resolved = null;

    public LibFilePath(LibFileFileSystem fileSystem, byte[] path) {
        this(fileSystem, path, false);
    }

    public LibFilePath(LibFileFileSystem fileSystem, byte[] path, boolean normalized) {
    ...
    @Override
    public LibFileFileSystem getFileSystem() {
        return fileSystem;
    }
    ...
    @Override
    public LibFilePath getRoot() {
        if (isAbsolute()) {
            return new LibFilePath(fileSystem, new byte[] {this.path[0]});
        }
        return null;
    }

    @Override
    public LibFilePath getFileName() {
        initOffsets();
        ...
        return new LibFilePath(fileSystem, path);
    }

    @Override
    public LibFilePath getParent() {
        initOffsets();
        int nbOffsets = offsets.length;
        if (nbOffsets == 0) {
            return null;
    ...
    @Override
    public LibFilePath getName(int index) {
        initOffsets();
        if (index < 0 || index >= offsets.length) {
            throw new IllegalArgumentException("Index " + index + " for " + this);
        }
    ...
    @Override
    public LibFilePath subpath(int beginIndex, int endIndex) {
        initOffsets();
    ...
    public boolean startsWith(Path other) {
        LibFilePath p1 = this;
        LibFilePath p2 = checkPath(other);
    ...
    @Override
    public LibFilePath normalize() {
        byte[] p = getResolved();
        if (p == this.path) {
            return this;
        }
        return new LibFilePath(fileSystem, p, true);
    }

    @Override
    public LibFilePath resolve(Path other) {
        LibFilePath p1 = this;
        LibFilePath p2 = checkPath(other);
        if (p2.isAbsolute()) {
            return p2;
        ...
        return new LibFilePath(fileSystem, result);
    }

    @Override
    public LibFilePath resolve(String other) {
    ...
    @Override
    public LibFilePath relativize(Path other) {
        LibFilePath p1 = this;
        LibFilePath p2 = checkPath(other);
        if (p2.equals(p1)) {
            return new LibFilePath(fileSystem, new byte[0], true);
        }
    ...
    @Override
    public LibFilePath toAbsolutePath() {
        if (isAbsolute()) {
            return this;
        }
        ...
        return new LibFilePath(fileSystem, result, true);
    }

    @Override
    public LibFilePath toRealPath(LinkOption... options) throws IOException {
        LibFilePath absolute = new LibFilePath(fileSystem, getResolvedPath()).toAbsolutePath();
        ...
        return new Iterator<Path>() {
            ...
            public Path next() {
                if (index < getNameCount()) {
                    LibFilePath name = getName(index);
                    index++;
                    return name;
                }
    ...
    public int compareTo(Path paramPath) {
        LibFilePath p1 = this;
        LibFilePath p2 = checkPath(paramPath);
        byte[] a1 = p1.path;
        byte[] a2 = p2.path;
        int l1 = a1.length;
        int l2 = a2.length;
    ...
    private LibFilePath checkPath(Path paramPath) {
        if (paramPath == null) {
            throw new NullPointerException();
        }
        if (!(paramPath instanceof LibFilePath)) {
            throw new ProviderMismatchException();
        }
        return (LibFilePath) paramPath;
    }
    ...
    public boolean equals(Object obj) {
        return obj instanceof LibFilePath
            && ((LibFilePath) obj).fileSystem == fileSystem
            && compareTo((LibFilePath) obj) == 0;
    }

    @Override
    public String toString() {
        return new String(path, StandardCharsets.ISO_8859_1);
    }
    ...
    private byte[] normalize(byte[] path) {
        ...
        for (int j = 0; j < path.length; j++) {
            ...
            if (k == 0) {
                throw new InvalidPathException(new String(path, StandardCharsets.ISO_8859_1), "Path: nul character not allowed");
            }
            i = k;
        }
        return path;
    ...
    private static byte[] doGetResolved(LibFilePath p) {
    ...
    private static boolean equalsNameAt(LibFilePath p1, LibFilePath p2, int index) {
