...
import me.nithanim.cultures.lsp.processor.services.*;
...
public class DefinitionEnvironment {
     ...
     */
    private final Map<SourceFile, List<? extends CulturesIniLine>> sourceFileToLines = new HashMap<>();
    /**
    ...
    @Autowired
    private SourceFileContentService sourceFileContentService;
    @Autowired
    private SourceFileParsingService sourceFileParsingService;
    @Autowired
    private DiagnosticsService diagnosticsService;
    @Autowired
    private PathResolverService pathResolverService;
    @Autowired
    private ConstantsResolverService constantsResolverService;
    @Autowired
    private CommandParameterLengthCheckService commandParameterLengthCheckService;
    ...
    private void updateFile(SourceFile sf, List<? extends CulturesIniLine> lines) {
        ...
        List<SourceFile> includes = lines.stream()
                .filter(l -> l instanceof CulturesIniInclude)
                .map(l -> (CulturesIniInclude) l)
                .map(l -> pathResolverService.resolvePartialPathToUri(l.getOriginPath().getSourceFile(), l.getPath()))
                .map(SourceFile::new)
    ...
    public List<? extends CulturesIniLine> calculateAllLines() {
        ...
        List<? extends CulturesIniLine> entryLines = sourceFileToLines.getOrDefault(pathResolverService.getEntry(), Collections.emptyList());
        ...
        while (backlog.hasNext()) {
            ...
            if(line instanceof CulturesIniInclude) {
                ...
                Uri resolved = pathResolverService.resolvePartialPathToUri(inc.getOriginPath().getSourceFile(), inc.getPath());
    ...
    private List<? extends CulturesIniLine> getLinesAndInitIfNeeded(SourceFile sourceFile) {
        ...
        if(l == null) {
            try {
                ...
                SourceFileParsingService.ParsingResult r = sourceFileParsingService.parseSourceFile(sourceFile, content);
                diagnosticsService.updateDiagnostics(sourceFile, DiagnosticsService.Type.PARSING, r.getDiagnostics());
                return r.getLines();
            } catch (IOException ex) {
    ...
    public void onSourceFileChanged(SourceFileChangedEvent evt) throws IOException {
        ...
        SourceFileParsingService.ParsingResult r = sourceFileParsingService.parseSourceFile(sourceFile, sourceFileContentService.getContent(sourceFile));
        diagnosticsService.updateDiagnostics(sourceFile, DiagnosticsService.Type.PARSING, r.getDiagnostics());
        ...
        List<? extends CulturesIniLine> allLines = commandParameterLengthCheckService.onData(getAllLinesResolved());
