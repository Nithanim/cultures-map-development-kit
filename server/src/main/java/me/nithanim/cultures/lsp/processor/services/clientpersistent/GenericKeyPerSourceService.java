...
public abstract class GenericKeyPerSourceService<
  ...
  private void insert(KEY_TYPE type, List<DATA_TYPE> things) {
    Map<SourceFile, List<DATA_TYPE>> bySourceFile =
        things.stream().collect(Collectors.groupingBy(d -> d.getOrigin().getSourceFile()));
  ...
  private void publish(SourceFile dirty) {
    ...
    List<DATA_TYPE> ds =
        keys.stream().map(things::get).flatMap(List::stream).collect(Collectors.toList());
    _publish(dirty, ds);
  }
