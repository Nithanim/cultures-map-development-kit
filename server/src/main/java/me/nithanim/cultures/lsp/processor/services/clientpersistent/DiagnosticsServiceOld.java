...
public class DiagnosticsServiceOld {
  ...
  public List<MyDiagnostic> getAllDiagnostics(SourceFile sourceFile) {
    ...
    if (keys != null) {
      return keys.stream().flatMap(k -> diagnostics.get(k).stream()).collect(Collectors.toList());
    } else {
      return null;
    }
  }
  ...
  public void updateDiagnostics(Type type, List<MyDiagnostic> diagnostics) {
    deleteDiagnostics(
        type); // new would overwrite old but if there are none they would never be removed
    insertDiagnostics(type, diagnostics);
  }

  private void insertDiagnostics(Type type, List<MyDiagnostic> diagnostics) {
    Map<SourceFile, List<MyDiagnostic>> bySourceFile =
        diagnostics.stream().collect(Collectors.groupingBy(d -> d.getOrigin().getSourceFile()));
    bySourceFile.forEach(
        (k, v) -> {
