...
public class CulturesIniDocumentService extends FullTextDocumentService {
    ...
    public CompletableFuture<Either<List<? extends Location>, List<? extends LocationLink>>> definition(TextDocumentPositionParams position) {
        logger.info("Req Definition for " + position.getTextDocument().getUri());
            //Location defLoc = culturesModel.findDefinition(position.getTextDocument().getSourceFile(), position.getPosition());
    ...
    public CompletableFuture<List<FoldingRange>> foldingRange(FoldingRangeRequestParams params) {
        ...
        String text = sourceFileContentService.getContent(new SourceFile(Uri.of(params.getTextDocument().getUri())));
