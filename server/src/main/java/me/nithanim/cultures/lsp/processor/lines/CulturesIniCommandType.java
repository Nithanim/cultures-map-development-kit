...
import static me.nithanim.cultures.lsp.processor.lines.CulturesIniCategoryType.*;
import static me.nithanim.cultures.lsp.processor.lines.CulturesIniCommand.Parameter.Type.*;

/**
 * Definitions of all commands available. Parameter names, types and basic
 * validation are in place.
 */
public enum CulturesIniCommandType {
    VERSION(LOGICCONTROL, pbn("Version", 1, 1)),
    MAPSIZE(LOGICCONTROL, pn(), pn()),
    MAPGUID(LOGICCONTROL, pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn(), pn()),
    //Map
    ...
    SETRAINRECTANGLE(MISC_WEATHER, pn("TopleftX"), pn("TopleftY"), pn("Bottomright"), pn("Bottomright"), pbn("Strength (100-5000)", 100, 5000)),
    TRADEAGREEMENT(MISC_TRADEAGREEMENT, pn("House id"), pn("Good type want"), pn("Amount want"), pn("Good type give"), pn("Amount give")),
    ...
    PLAYEROPTION(MULTIPLAYER, 1, 3, pplayer(), pbn("Player type", 0, 2), pbn("Player type", 0, 2), pbn("Player type", 0, 2)),
    ...
    //StaticObjects
    SETHOUSE(STATICOBJECTS, pplayer(), pt("House type"), pn("Level"), pbn("Finished", 0, 1), pn("PosX"), pn("PosY"), pn("Id")),
    SETVEHICLE(STATICOBJECTS, pplayer(), pt("Tribe"), pt("Vehicle"), pn("PosX"), pn("PosY"), pn("Id")),
    ADDGOODS(STATICOBJECTS, pt("Good"), pn("Amount")),
    SETHUMAN(STATICOBJECTS, pplayer(), pt("Tribe"), pt("Job"), pn("PosX"), pn("PosY"), pn("Id"), pn("Behavior")),
    ...
    CHILDOFWOMAN(STATICOBJECTS, pn("Child PosX"), pn("Child PosY"), pn("Woman PosX"), pn("Woman PosY")),
    SETANIMAL(STATICOBJECTS, pplayer(), pt("Tribe"), pt("Job"), pn("PosX"), pn("PosY"), pn("Id"), pn("Behavior")),
    ...
    AI_SETCONDITION_ONCONDITIONS(AIDATA, 5, -1, pplayer(), pn("Target condition id"), pbn("Ever", 0, 1), pn("Logical concatenation")),
    AI_SETCONDITION_ONCONDITIONSCHANGEDELAYED(AIDATA, pplayer(), pn("Target condition id"), pbn("Ever", 0, 1), pn("Source condition to wait for true"), pbn("Target state", 0, 1), pn("Delay in seconds")),
    AI_SETCONDITION_ONDIPLOMACYCHANGE(AIDATA, pplayer(), pn("Target condition id"), pbn("Ever", 0, 1), pbn("diplomacy", 1, 3)),
    AI_SETCONDITION_ONCREATUREINRANGE(AIDATA, pplayerA(), pn("Target condition id"), pbn("Ever", 0, 1), pn("PosX"), pn("PosY"), pn("Radius"), pplayerB(), pbn("Second play needs to be an enemy to first player?", 0, 1), pbn("Count soldiers only", 0, 1)),
    ...
    CulturesIniCommandType(CulturesIniCategoryType category, int paramMin, int paramMax, ParameterInfo... parameterTypes) {
        this(false, category, parameterTypes.length, parameterTypes.length, parameterTypes);
    }
    ...
    CulturesIniCommandType(boolean special, CulturesIniCategoryType category, int paramMin, int paramMax, ParameterInfo... parameterTypes) {
    ...
    /**
     * Sets the parameter as specific number which allows only values given by
     * the array.
     *
     * @param name    the parameter name
