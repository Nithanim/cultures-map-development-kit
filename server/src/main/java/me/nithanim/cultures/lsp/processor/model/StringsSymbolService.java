...
public class StringsSymbolService implements StringsSymbolTable {
    ...
    public void replaceLanguage(List<CulturesIniCommand> commands) {
        Map<Boolean, List<CulturesIniCommand>> collect = commands.stream().collect(Collectors.groupingBy(c -> c.getCommandType() == CulturesIniCommandType.STRINGN));

        //disgnosticsService.replaceDiagnostis();
        collect.get(Boolean.FALSE).stream()
                .map(c -> new MyDiagnostic(c.getOriginAll(), DiagnosticSeverity.Error, "Only stringn allowed"))
